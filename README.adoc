...
== Bebop Converter (Observability Channel)

Convert NDJSON events into Bebop frames when `bebopc` is available:
...
Schemas live at `bebop/schemas/ambientops_events.bop`.

If `bebopc` is not available, the converter writes JSON payloads instead.
...
See link:docs/SYSTEM_OVERVIEW.adoc[docs/SYSTEM_OVERVIEW.adoc] for data flow and boundaries.
